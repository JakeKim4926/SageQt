...
SageQt는 Windows 전용 MFC 앱 SageSDI를 대체하는 **단일 Qt 코드베이스**다. 배경과 결정 이력은 `docs/decisions/MIGRATION_PLAN.md`에 있다.

1. **macOS를 주 대상으로, Windows · Linux까지 — 코드는 하나, 세팅만 다르다.** OS별 차이는 CMake 세팅과 패키징에만 둔다
2. **디자인 자유도.** 플랫폼 모양에 묶이지 않는 UI를 만든다
3. **품질 = 일관성.** 이름 · 배치 · 클래스 간 규칙이 늘 통일되어 있고, 유지보수가 쉬운 구조를 지킨다. 비용보다 품질을 우선한다

작업이 이 목표와 어긋나 보이면 멈추고 먼저 확인한다. 세부 규칙은 `.claude/skills/`에 있다.
지금 할 일은 `sageqt-plan` 스킬의 *진행 중인 주제* 목록에 있다 — 작업을 시작 · 재개할 때 먼저 확인한다.
