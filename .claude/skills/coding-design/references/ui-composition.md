...
**판단 기준: 워크플로를 하나 추가할 때 `core/workflow/handlers/`에 새 파일을 만들고
중앙 등록부 1곳만 고치면 되는가.**
...
SageSDI에서 저장 위치 컨트롤이 결과 탭에서도 보이는데 배치는 입력 탭에서만 계산되어 결과 영역과 겹친 사례가 있다.
...
- 워크플로 1종 추가 = `handlers/`에 파일 1쌍 + 등록부 1곳 (사이드바 포함 다른 곳은 수정 없음)
...
줄 수 · 연결 수 참고값은 Qt 코드가 생긴 뒤 실측해서 정한다. SageSDI의 MFC 수치를 그대로 적용하지 않는다.
